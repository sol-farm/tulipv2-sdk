...
describe('examples', () => {
  ...
  it('issues shares', async () => {
    ...
    let tx = await program.rpc.issueShares(new anchor.BN(1), [new anchor.BN(1), new anchor.BN(65537)], {accounts: {
        authority: provider.wallet.publicKey,
        vault: usdcv1Vault,
        depositTrackingAccount,
        depositTrackingPda,
  ...
  it("withdraws multi deposit vault through mango", async() => {
    let tx = await program.rpc.withdrawMultiDepositVaultThroughMango(new anchor.BN(0), {accounts: {
      commonData: {
        authority: provider.wallet.publicKey,
  ...
  it("withdraws multi deposit vault through solend", async() => {
    let tx = await program.rpc.withdrawMultiDepositVaultThroughSolend(new anchor.BN(0), {accounts: {
      commonData: {
        authority: provider.wallet.publicKey,
        multiVault: usdcv1Vault,
        multiVaultPda: usdcv1VaultPda,
  ...
  it("withdraws multi deposit vault through solend", async() => {
    let tx = await program.rpc.withdrawMultiDepositVaultThroughTulip(new anchor.BN(0), {accounts: {
      commonData: {
        authority: provider.wallet.publicKey,
        multiVault: usdcv1Vault,
        multiVaultPda: usdcv1VaultPda,
